	...
	function get_All($limit = 10, $offset = 0) 
  	{
		...
		if($level=="5"){ $qt="WHERE author = '$userid'"; }
  		$query =$this->db->query("select * from belajar $qt ORDER BY id_belajar DESC LIMIT $offset,$limit");
		return $query->result();
  	}
	function get_All2($limit = 10, $offset = 0) 
  	{
		if($offset==""){ $offset=0; }
  		$query =$this->db->query("select * from belajar WHERE status = 'publish' ORDER BY id_belajar DESC LIMIT $offset,$limit");
		return $query->result();
  	}
	function getProgramBaru()
	{
		$query =$this->db->query("select * from belajar WHERE status = 'publish' ORDER BY id_belajar DESC LIMIT 1");
		return $query->result();
	}
