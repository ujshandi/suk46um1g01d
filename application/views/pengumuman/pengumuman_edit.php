			...
			alert('Harap Isi Semua Field (*) !!');
				...
				<h1 class="titlebig">Edit pengumuman</h1>
					<div class="boxbigcontent">
						...
						<form method="post" action="<?=base_url()?>pengumuman/simpanedit"  id="frmcontact">
						<?php foreach($pengumuman as $row): ?>
							<input type="hidden" name="id" value="<?=$row->id_pengumuman?>" />
							<label style="margin-left:-15px;">Judul</label><br />
							<input type="text" name="judul" id="f1" class="textboxcontact" size="80" value="<?=$row->judul?>" /> <span>*</span><br />
							<label style="margin-left:-15px;">Isi Pengumuman</label><br /><br />
							<textarea name="deskripsi" id="editor1" rows="22">
							<?=$row->deskripsi?>
							</textarea>
							<br /><br />
						<?php endforeach; ?>
